...
@interface ExtremeCombiningCharactersValidator : NSObject <ZMPropertyValidator>
...
- (BOOL)zm_conformsTo:(CFStringRef _Nonnull)uti SWIFT_WARN_UNUSED_RESULT;
@end
...
- (NSComparisonResult)compareWithType1UUID:(NSUUID * _Nonnull)type1UUID SWIFT_WARN_UNUSED_RESULT;
...
+ (BOOL)validateValue:(id _Nullable * _Nonnull)ioValue minimumStringLength:(uint32_t)minimumStringLength maximumStringLength:(uint32_t)maximumStringLength maximumByteLength:(uint32_t)maximumByteLength error:(NSError * _Nullable * _Nullable)error;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
