...
@interface IMKXSocketRocketUtility()<SRWebSocketDelegate>
{
    int _index;
    NSTimer * heartBeat;
...
@implementation IMKXSocketRocketUtility

+(IMKXSocketRocketUtility *)instance{
    static IMKXSocketRocketUtility *Instance = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        Instance = [[IMKXSocketRocketUtility alloc] init];
    });
    return Instance;
}
...
//    }
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(reConnectTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self SRWebSocketOpenWithURLString:[KSocketUrl stringByAppendingString:token]];
        NSLog(@"socket重连===");
    });
...
-(void)sentheart{
    ...
    [self sendData:@"IMKeXin is so much nicer with internet!"];
}
